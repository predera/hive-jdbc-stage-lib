...
public class BoneCPPoolConfigBean {
  ...
  public static final String BONECP_BEAN_NAME = "bonecpConfigBean.";
...
*/
  ...
  public List<Stage.ConfigIssue> validateConfigs(Stage.Context context, List<Stage.ConfigIssue> issues) {
    ...
    if (!driverClassName.isEmpty()) {
      try {
        ...
        issues.add(context.createConfigIssue(com.streamsets.pipeline.stage.origin.hive_jdbc.Groups.LEGACY.name(), DRIVER_CLASSNAME, JdbcErrors.JDBC_28, e.toString()));
      }
    }
  ...
  public Properties getDriverProperties() throws StageException {
    ...
    for (ConnectionPropertyBean bean : driverProperties) {
      properties.setProperty(bean.key, bean.value);
    }
    return properties;
  }

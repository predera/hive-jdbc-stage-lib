...
import com.streamsets.pipeline.stage.origin.hive_jdbc.cdc.TableOrderStrategyChooserValues;
...
public class CDCTableJdbcConfigBean {
  ...
  public List<Stage.ConfigIssue> validateConfigs(PushSource.Context context, List<Stage.ConfigIssue> issues) {
    if (tableConfigs.isEmpty()) {
      issues.add(context.createConfigIssue(com.streamsets.pipeline.stage.origin.hive_jdbc.table.Groups.TABLE.name(), TABLE_CONFIG, JdbcErrors.JDBC_66));
    }
    if (batchTableStrategy == BatchTableStrategy.SWITCH_TABLES && numberOfBatchesFromRs == 0) {
      issues.add(
          context.createConfigIssue(
              com.streamsets.pipeline.stage.origin.hive_jdbc.table.Groups.JDBC.name(),
